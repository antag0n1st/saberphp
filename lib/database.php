...
//        }
    ...
    private function execute() {
        ...
        if (self::$isLogging) {
            $msc = microtime(true);
            $this->result = mysqli_query(Database::$con, $this->sql) or die((HOST_ID != 0) ? "MySQL ERROR: " . mysqli_error(Database::$con) . " QUERY: " . $this->sql : "");
            self::$queries[] = array('query' => $this->sql, 'time' => microtime(true) - $msc);
        } else {
            $this->result = mysqli_query(Database::$con, $this->sql) or die((HOST_ID != 0) ? "MySQL ERROR: " . mysqli_error(Database::$con) . " QUERY: " . $this->sql : "");
        }
    }
